...
def run_recorder(command_queue: Queue, result_queue: Queue, sample_rate: int = 16000) -> None:
    ...
    try:
        ...
        # Stop recording - just stop, don't close yet (close can hang on CoreAudio)
        recording = False
        if stream:
            stream.stop()
        ...
        # Send result BEFORE stream.close() - close can hang on CoreAudio
        result_queue.put(tmp_path)

        # Now close stream - if it hangs, main thread already has the result
        if stream:
            stream.close()
        ...
        # Last resort cleanup - stream.close() might hang, but process will be killed anyway
        if stream:
            try:
                stream.close()
            except Exception:

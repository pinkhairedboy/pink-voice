...
class BaseUI(ABC):
    ...
    def _process_recording(self, audio_path: Optional[str]) -> None:
        ...
        try:
            ...
            self.on_transcription_success(text_with_disclaimer)
            self.play_sound("done")
            self.copy_to_clipboard(text_with_disclaimer)
            self.show_notification(
                "Done",
                text[:100] + ("..." if len(text) > 100 else "") if text else "No speech detected"
            )
    ...
    def on_transcription_success(self, text: str) -> None:
        ...
        Args:
            text: Transcribed text with disclaimer
        """
        pass
